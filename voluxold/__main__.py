...
class BrightnessMode:
    ...
    def vacate(self):
        ...
        if mixer.gmute() is True:
            return MuteMode

        elif mixer.gmute() is False:
            return VolumeMode
...
class MainApplication(tk.Frame):
    ...
    def _init_objects(self):
        ...
        self.displaySize = VolAs._get_display_size(
            root
        )  # max size of the bar in pixels

        self.barWidth = LivePercentage(
            0, self.displaySize["x"]
        )  # set width of bar
    ...
    def _init_window(self):
        ...
        self.barContainer = tk.Frame(m, style="barContainer.TFrame")
    ...
    def _update_bar(self):
        ...
        mode_style_id = (
            VolBar.mode.style_id
        )  # set background based on mode color
        ...
        newWidth = (
            self.barWidth.getNum()
        )  # get a numerical version of the percentage

        self.bar.configure(
            style=mode_style_id, width=str(newWidth)
        )  # update the bar with these settings

    def _update_volume(self):
        mixer.svol(VolAs.volume)

    def _toggle_mute(self):
        mixer.smute(not mixer.gmute())  # toggle mute state

    def _mouse_entered(self, event):
        root.wm_attributes("-alpha", preferences["default_opacity"])

    def _mouse_left(self, event):
        root.wm_attributes("-alpha", preferences["outside_zone_opacity"])
...
windowOffsets = {"x": 0, "y": dispSize["y"] - mainApp.barHeight}
root.geometry(
    "{}x{}+{}+{}".format(
        overlay_w, overlay_h, windowOffsets["x"], windowOffsets["y"]
    )
)  # define the size of the window
root.attributes(
    "-topmost", True
)  # force window to stay on top (doesn't work in full screen applications)
root.overrideredirect(1)  # remove frame of window
root.wait_visibility(root)  # required for window transparency
root.wm_attributes(
    "-alpha", preferences["outside_zone_opacity"]
