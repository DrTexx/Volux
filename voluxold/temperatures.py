...
def print_core(core, show_percentages=True):
    ...
    print(
        "[CORE]: {:<3} | [TEMP]: {} | [HIGH]: {} | [CRITICAL]: {}".format(
            core.label, core.current(), core.high, core.critical
        )
    )
    if show_percentages is True:
        print(
            "--- [{}%] of throttle heat ({}C)".format(
                round(high_percent, 2), core.high
            )
        )
        print(
            "--- [{}%] of max heat ({}C)".format(
                round(critical_percent, 2), core.critical
            )
        )
    ...
    def _start_thread(self):
        ...
        while self.running is True:
            ...
            for core in self.cores:
                now = time.localtime()
                timestring = "{:>2}:{:>2}:{:>2}".format(now[3], now[4], now[5])
                ...
                if core.current() >= core.critical:
                    ...
                    self._core_abnormal(core)
            print(
                "MEMORY USED: [{:<4}%] Virtual | [{:<4}%] Swap".format(
                    psutil.virtual_memory()[2], psutil.swap_memory()[3]
                )
            )

    def start(self):
        if self.running is True:
            print("CoreWatch already running!")
        else:
